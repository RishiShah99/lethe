"""K#1 inc-B2 v3 — fused reverse-scan kernel: offset-partition lifecycle + UNROLLED chunks.

The burst-2 v2 attempt (scratch/k1_incb2_offsets.py history) kept a real ``scf.for`` over
chunks and hoisted the epilogue ``make_tmem_copy`` atoms pre-loop — box-adjudicated ICE
("failed to legalize unresolved materialization ... remained live after conversion"): the
mamba2_ssd pre-loop-atom idiom covers its STORE atoms, not our LOAD ``tmem_copy`` in this
config. v3 is the documented fallback: ``cutlass.range_constexpr(nt)`` UNROLLS the chunk
loop so every chunk is the silicon-proven straight-line body (atoms consumed outside any
scf region); the K-tile mainloops stay real dynamic loops. Everything else is v2 verbatim:
ONE tmem.allocate(128), acc0/acc1 at column offsets 0/64 (tmem_offset_probe.py GO), one
relinquish + one free at the end, hoisted L-free TMA partitions sliced per chunk, two
mma objects + two acc mbarrier sets, epilogue sync on barrier_id=2, store->fence->barrier
ordering on both GMEM round-trips (b_ga second half; b_dh/b_dhT carry).

Cost model: one executable per (n_bh, nt, c, d_v, cw) — nt is baked; IR size grows with
nt (gate nt in {1, 4} before bench-scale nt=32; compile time is the known risk, not
correctness). Per-chunk TMEM readback is structural (the carry feeds the next chunk's
GEMM operands through SMEM/GMEM), so unrolling — not a dynamic loop — is the only legal
shape for it on this wheel.
...
Constexpr ``nt``: one compiled executable per (n_bh, nt, c, d_v, cw); the chunk loop is
FULLY UNROLLED (range_constexpr) and only the K-mainloops stay dynamic.  ``d_k`` is fixed
at 128 (tile M), ``c`` at 64 (half-K), both enforced in ``run_k1_incB2_v3``.

ADVERSARIAL-REVIEW REPAIRS (2 skeptics + arbiter, desk, pre-box):
  * FATAL — ab_e.count as the gmem K coordinate: count is MONOTONIC across the 2*nt
    mainloops on one participant pair (mamba2_ssd reset_count()-per-work-tile proves the
    semantics); K-rest extent is statically 1 → OOB from chunk-0's GA onward, silently
    wrong + bit-deterministic.  All four slices now use literal 0.
  * FATAL — ex(*args) re-passed the 4 trailing Constexprs; the 4.5.2 executor drops them
    from the runtime signature and appends extras → pointer-slot shift → host SIGSEGV at
    launch (v0's exact banked symptom — the entire v0 bisect SIGSEGV matrix is plausibly
    THIS launcher fault, not kernel machinery).  Call is now ex(*args[:16]).
...
SCOPE CONSTRAINT — single-N-tile only (d_v = 64):
  ...
  never written.  ``run_k1_incB2_v3`` raises ``ValueError`` when ``d_v != 64``.
...
Box gate: ``run_k1_incB2_v3`` → ``_kern_incb2_v3`` on B200; grade dh/dv2/dh0
...
OPEN RISKS (box adjudicates):
  (a) IR/compile-time growth: the unroll instantiates nt copies of both mainloops +
      epilogues + SIMT glue. Gate nt in {1, 4} first; nt=32 (L=2048 bench scale) is the
      compile-time stress case. If compile time explodes, split bench shapes across two
      launches (two nt=16 executables chained through the same GMEM carry) — the carry
      round-trip already goes through GMEM, so chaining is value-identical.
  (b) The single-phase acc pipeline (num_stages=1) is acquired/committed nt times in a
      straight line — matches the v0 committed kernel's unrolled pattern; the offset
      lifecycle underneath is what tmem_offset_probe.py proved GO. If the phase toggles
      misalign across 2*nt cycles, the symptom is a hang (mbarrier deadlock), not a
      wrong answer — the 2-run determinism check cannot mask it.
"""
...
def run_k1_incB2_v3(
    ...
    *,
    ...
    ``_kern_incb2_v3`` once for all n_bh groups. Returns ``(dh, dv2, dh0)`` head-major.
    ...
    # runtime signature — re-passing them shifts the executor's pointer slots (F1).
    ex(*args[:16])
...
def _build_bundle(mode: str, nt: int = 4) -> dict[str, Any]:
    """Build an in-process K#1 bundle at (b=2,h=2,nt,c=64,d_k=128,d_v=64); nt is the stage knob."""
    from flash_mamba_rl.kernels.references.gdn2_chunkwise import build_microgate_bundles
    from flash_mamba_rl.kernels.references.gdn2_chunkwise_cw import build_microgate_bundles_cw

    b, h, c, d_k, d_v = 2, 2, 64, 128, 64
    t = nt * c
