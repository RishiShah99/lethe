...
class TestDefaultScanMode:
    def test_forward_short_l_is_serial_every_batch_width(self) -> None:
        # The forward kernel is cheap per step; below L=512 the carry never pays, at any tested shape.
        for batch in (1, 2, 8):
    ...
    def test_backward_short_l_small_batch_is_chunk_parallel(self) -> None:
        # Backward favors serial only when saturated; small batch at short L is still a chunk_parallel win.
...
class TestMemoryBoundFallback:
    ...
    def test_scratch_bytes_formula_matches_block_sizing(self) -> None:
        # Mirrors kernel scratch formula: hbuf_elems = batch*n_chunks*n_d_blocks*chunk_len*block_d*block_n
    ...
    def test_large_n_state_uses_more_memory(self) -> None:
        # At larger n_state, block_d shrinks but block_n and n_d_blocks grow, so scratch is larger.
    ...
    def test_selector_keeps_chunk_parallel_when_n_state_missing(self) -> None:
        # Without n_state, the selector cannot check memory; it uses shape-based heuristics only.
    ...
    def test_selector_returns_serial_for_overcap_n_state(self) -> None:
        # The chunk-parallel backward launchers reject n_state > 128; the selector must not route there.
    ...
    def test_selector_returns_serial_for_memory_exceeding_shape(self) -> None:
        # A shape whose scratch need exceeds any plausible GPU memory budget must fall back to serial.
        import torch
    ...
    def test_selector_still_picks_chunk_parallel_for_small_shapes(self) -> None:
        # A typical training shape should still route to chunk_parallel under the memory check.
...
class TestAgainstBoundarySweep:
    ...
    def test_no_bad_regression_when_routing_chunk_parallel(self) -> None:
        # Wherever the rule picks chunk_parallel, it's never materially slower than serial (worst ~0.955).
        for op, b, length, w, bs, bc in self._rows():
            mode = _default_scan_mode(length, b, w, is_forward=(op == "forward_chunked_scan"))
