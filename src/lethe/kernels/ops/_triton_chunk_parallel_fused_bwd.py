...
from lethe.kernels.ops._triton_bwd_scan import MAX_BLOCK_N
from lethe.kernels.ops._triton_chunk_parallel_bwd import _rev_carry_scan
from lethe.kernels.ops._triton_chunk_parallel_fwd import _carry_scan
from lethe.kernels.ops._triton_fused_block import (
    _MAX_BLOCK_D_NORM,
    _NORM_TILE,
    MAX_CONV_K,
)
from lethe.kernels.ops._triton_fused_block_bwd import (
    _conv_x_bwd_kernel,
    _norm_bwd_kernel,
)
    ...
    *,
    ...
    if x.is_cuda:
        ...
        if scratch_bytes > 0.7 * free_bytes:
            raise ValueError(
                f"chunk_parallel C6 backward scratch ~{scratch_bytes / 1e9:.1f} GB exceeds "
