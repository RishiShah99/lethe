...
the training shape, so an off-default / long-L target leaves real headroom
(a grid sweep measures 1.5-1.7x on fused_block_forward).
...
def serial_seed_completions(op: str, n: int) -> list[str]:
    ...
    Replacing the first ``n`` of the K sampled completions with these makes
    the reward see serial: at a saturated shape serial out-scores
    chunk_parallel so its group-relative advantage is
