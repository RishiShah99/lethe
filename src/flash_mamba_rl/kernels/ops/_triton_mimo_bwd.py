...
write fp32 partials. State recompute follows C2: a forward sweep
checkpoints the state entering each K-step chunk, chunks are processed
newest-first with an in-chunk forward recompute of the pre-update states
into an L2-resident scratch, then a reverse sweep walks the chunk. The
