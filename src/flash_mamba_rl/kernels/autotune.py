...
class KernelConfig:
    ...
    def searched(self) -> dict[str, int]:
...
SEARCH_GRID: dict[str, dict[str, tuple[int, ...]]] = {
    "forward_chunked_scan": {"block_d": _BLOCK, "num_warps": _WARPS, "num_stages": _STAGES},
...
def validate(op: str, config: KernelConfig, *, shape: ShapeSpec | None = None) -> list[str]:
    ...
    violation. With ``shape`` given, ``chunk_k`` must divide ``seq_len`` (the
    checkpoint window tiles the sequence).
    """
...
def iter_configs(op: str) -> Iterator[KernelConfig]:
    ...
    for combo in itertools.product(*(grid[k] for k in keys)):
        yield KernelConfig(**dict(zip(keys, combo, strict=True)))
